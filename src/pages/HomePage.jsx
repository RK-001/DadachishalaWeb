...
import { useEvents, useGalleryItems, useSuccessStories, useTestimonials, useAwards } from '../hooks/useFirebaseQueries';
...
const SectionHeader = memo(({ badge, badgeIcon, title, description, light = false }) => (
  <div className="text-center mb-8 sm:mb-10 px-2">
    <div className={`inline-flex items-center space-x-2 ${light ? 'bg-white/10 text-white' : 'bg-primary-100 text-primary-600'} rounded-full px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-medium mb-3`}>
      {badgeIcon}<span>{badge}</span>
    </div>
    <h2 className={`text-2xl sm:text-3xl md:text-4xl font-heading font-bold ${light ? 'text-white' : 'text-primary-600'} mb-3 sm:mb-4 px-4`}>{title}</h2>
    <p className={`text-base sm:text-lg ${light ? 'text-gray-100' : 'text-gray-600'} max-w-3xl mx-auto px-4`}>{description}</p>
  </div>
));
...
const HomePage = () => {
  ...
  const { data: sliderImages = [], isLoading: sliderLoading } = useGalleryItems('home-slider', 5);
  ...
  const eventsCount = useMemo(() => Math.min(8, events.length), [events.length]);

  // Custom hooks for auto-scroll
  const testimonialNav = useAutoScroll(testimonialRef, testimonials.length, 374, 4000, currentTestimonial, setCurrentTestimonial);
  ...
  // Mark images loaded when slider images are available
  useEffect(() => {
    if (!sliderLoading && sliderImages.length > 0) {
      setImagesLoaded(true);
    }
  }, [sliderLoading, sliderImages.length]);

  // Auto-rotate slides
  useEffect(() => {
    if (!imagesLoaded || sliderImages.length <= 1) return;
    const timer = setInterval(() => setCurrentSlide(p => (p + 1) % sliderImages.length), 5000);
    return () => clearInterval(timer);
  }, [sliderImages.length, imagesLoaded]);

  const handleSlideChange = useCallback((dir) => {
    setCurrentSlide(p => dir === 'prev' 
      ? (p - 1 + sliderImages.length) % sliderImages.length 
      : (p + 1) % sliderImages.length);
  }, [sliderImages.length]);
  ...
  const homeSchema = {
    ...
    "@type": "WebSite",
    "name": "Dada Chi Shala",
    "alternateName": "Dadachishala",
    "url": "https://dadachishala.org",
    "description": "Leading NGO in Pune providing free education to street children",
    "potentialAction": {
      "@type": "SearchAction",
      "target": "https://dadachishala.org/search?q={search_term_string}",
  ...
  return (
    <>
      <SEO
        title="Dada Chi Shala - Free Education for Street Children in Pune | NGO Maharashtra"
        description="Dada Chi Shala educates 450+ street children across 10 branches in Pune. We've mainstreamed 524+ students into schools and rescued 257+ from begging. Support education in Pune today!"
        keywords="Dadachishala, Dada Chi Shala, street children education Pune, NGO Pune, free education Maharashtra, underprivileged children, donate education, volunteer Pune, slum education, best NGO Pune"
        canonicalUrl="/"
        structuredData={homeSchema}
      />
      <div className="overflow-hidden">
      {/* Hero Section */}
      <section className="relative min-h-[80vh] sm:min-h-screen bg-primary-600 overflow-hidden">
        {/* Background */}
            ...
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
              <div className="text-white space-y-6 sm:space-y-8 px-2 sm:px-4 md:px-6 lg:pl-12">
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                  <Link to="/donate" className="group bg-white text-primary-600 hover:bg-yellow-300 hover:text-primary-700 px-6 sm:px-8 py-3 sm:py-4 rounded-lg sm:rounded-xl font-bold text-base sm:text-lg transition-all duration-300 transform hover:scale-105 hover:shadow-2xl inline-flex items-center justify-center">
                    <Heart className="mr-2 sm:mr-3 w-4 h-4 sm:w-5 sm:h-5 group-hover:animate-pulse" />Donate Now<ArrowRight className="ml-2 sm:ml-3 w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform" />
                  </Link>
                  <Link to="/volunteer" className="group border-2 border-white text-white hover:bg-white hover:text-primary-600 px-6 sm:px-8 py-3 sm:py-4 rounded-lg sm:rounded-xl font-bold text-base sm:text-lg transition-all duration-300 transform hover:scale-105 inline-flex items-center justify-center">
                    <Users className="mr-2 sm:mr-3 w-4 h-4 sm:w-5 sm:h-5" />Join Our Mission
                  </Link>
                </div>
      ...
      {/* Impact Statistics */}
      <section className="py-8 sm:py-12 bg-gradient-to-r from-white to-gray-50 border-t border-primary-100">
        <div className="container-custom">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {IMPACT_STATS.map((stat, i) => (
              <div key={i} className="group bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100">
                <div className="relative mb-3 sm:mb-4">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-primary-500 to-primary-600 rounded-full mx-auto flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-300">
                    <span className="text-lg sm:text-xl text-white">⭐</span>
                  </div>
                </div>
                <h3 className="font-semibold text-gray-900 text-sm sm:text-base leading-tight">{stat.label}</h3>
              </div>
            ))}
          </div>

          <div className="text-center mt-6 sm:mt-8">
            <Link to="/about" className="inline-flex items-center space-x-2 bg-primary-600 hover:bg-primary-700 text-white px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl text-sm sm:text-base font-semibold transition-all duration-300 transform hover:scale-105 hover:shadow-lg">
              <span>Learn More About Our Impact</span><ArrowRight className="w-4 h-4 sm:w-5 sm:h-5" />
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-10 sm:py-14 bg-gradient-to-br from-primary-700 via-primary-600 to-primary-800 relative overflow-hidden">
        {/* Animated Background Elements */}
        <div className="absolute inset-0">
          {/* Floating circles */}
          <div className="absolute top-20 left-10 w-48 h-48 sm:w-64 sm:h-64 bg-yellow-400/10 rounded-full blur-3xl animate-pulse" />
          <div className="absolute bottom-20 right-10 w-64 h-64 sm:w-96 sm:h-96 bg-white/5 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] sm:w-[600px] sm:h-[600px] bg-primary-400/10 rounded-full blur-3xl" />
          {/* Pattern overlay */}
        ...
        <div className="container-custom relative z-10">
          {/* Enhanced Header */}
          <div className="text-center mb-8 sm:mb-10 px-2">
            <div className="inline-flex items-center space-x-2 bg-white/15 backdrop-blur-sm text-white rounded-full px-4 sm:px-5 py-1.5 sm:py-2 text-xs sm:text-sm font-medium mb-3 sm:mb-4 border border-white/20 shadow-lg">
              <Sparkles className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-300 animate-pulse" />
              <span>Events & Activities</span>
              <Sparkles className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-300 animate-pulse" />
            </div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-heading font-bold text-white mb-3 sm:mb-4 px-4">
            ...
            <p className="text-sm sm:text-base lg:text-lg text-gray-100 max-w-2xl mx-auto px-4">
          ...
          {loading.events ? (
            <div className="text-center py-8">
              <div className="relative inline-block">
                <div className="animate-spin rounded-full h-10 w-10 sm:h-12 sm:w-12 border-4 border-white/20 border-t-yellow-300 shadow-lg" />
                <Calendar className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-white" />
              </div>
            ...
            <>
              <div ref={eventsRef} className="overflow-x-auto scrollbar-hide pb-6 scroll-smooth snap-x snap-mandatory">
                <div className="flex space-x-6 min-w-max px-4">
                  {processedEvents.slice(0, 8).map((event, i) => (
        ...
        <div className="container-custom relative z-10">
          ...
          {stories.length > 0 ? (
            <>
              ...
              {/* Navigation controls */}
              <ScrollNav count={stories.length} current={currentStory} onNavigate={storiesNav.navigate} onGoTo={storiesNav.goTo} />
              <div className="text-center mt-8">
          ...
          {testimonials.length > 0 ? (
            <>
              ...
              {testimonials.length > 1 && (
                <ScrollNav count={testimonials.length} current={currentTestimonial} onNavigate={testimonialNav.navigate} onGoTo={testimonialNav.goTo} light />
              )}
            </>
          ) : (
            <div className="text-center py-10">
        ...
        <div className="container-custom relative z-10">
          ...
          {processedAwards.length > 0 ? (
            <>
              ...
              {/* Navigation controls */}
              <ScrollNav count={processedAwards.length} current={currentAward} onNavigate={awardsNav.navigate} onGoTo={awardsNav.goTo} />
